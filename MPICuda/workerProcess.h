...
public:
    ...
    std::vector<float> part1() override {
            ...
            timeMaster[0] = (endTime - startTime) * 1000; 
            ...
            endTime = MPI_Wtime(); //timekernel host 
            timeMaster[1] = (endTime - startTime) * 1000; 
            ...
            MPI_Send(&timeMaster[0], timeMaster.size(), MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
...
private:
    std::vector<float> timeMaster {4,0};// the timeSend vector is like this [inputPrepHost, outputPrepHost, calcTimeHost, calcTimeDevice]
